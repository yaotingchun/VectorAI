...
const rawCode = fs.readFileSync(path.join(root, 'src/features/machines/data/seedMachines.ts'), 'utf-8');
const start = rawCode.indexOf('= [') + 2;
const end = rawCode.lastIndexOf(']');
const jsonMatch = rawCode.slice(start, end + 1);
const allMachines = JSON.parse(jsonMatch);
...
console.log('--- CRITICAL MACHINES (2) ---');
critical.forEach(m => {
